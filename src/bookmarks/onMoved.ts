...
function listenerWrapper(constructor: any, method: AllowedListener, propertyKey: string | symbol, options: { filter?: (arg: { id: string, moveInfo: browser.Bookmarks.OnMovedMoveInfoType }) => boolean | Promise<boolean> } = {}) {
 ...
 */
export function onBookmarkMoved<T extends AllowedListener>({ filter }: { filter?: (arg: { id: string, moveInfo: browser.Bookmarks.OnMovedMoveInfoType }) => boolean | Promise<boolean> } = {}) {
