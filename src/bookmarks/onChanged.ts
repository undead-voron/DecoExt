...
function listenerWrapper(constructor: any, method: AllowedListener, propertyKey: string | symbol, options: { filter?: (arg: { id: string, changeInfo: browser.Bookmarks.OnChangedChangeInfoType }) => boolean | Promise<boolean> } = {}) {
 ...
 */
export function onBookmarkChanged<T extends AllowedListener>({ filter }: { filter?: (arg: { id: string, changeInfo: browser.Bookmarks.OnChangedChangeInfoType }) => boolean | Promise<boolean> } = {}) {
